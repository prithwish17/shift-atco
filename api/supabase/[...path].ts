...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (req.method === 'OPTIONS') {
        ...
        res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, apikey, x-client-info, prefer, x-upsert');
    ...
    // Forward x-upsert header — required for Supabase Storage upsert operations
    if (req.headers['x-upsert']) {
        headers['x-upsert'] = req.headers['x-upsert'] as string;
    }
    ...
    try {
        ...
        if (req.method !== 'GET' && req.method !== 'HEAD' && req.body) {
            ...
            if (isBinary && Buffer.isBuffer(req.body)) {
                // Binary uploads (e.g. Storage): forward the raw Buffer as-is.
                // JSON.stringify on a Buffer produces {"type":"Buffer","data":[...]} which corrupts the file.
