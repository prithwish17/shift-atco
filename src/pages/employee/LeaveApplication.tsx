  ...
  return (
    <DashboardLayout role="employee">
      <div className="space-y-6">
        <div>
          <h1 className="text-xl font-bold sm:text-2xl">Leave Management</h1>
          <p className="text-xs text-muted-foreground sm:text-sm">Apply for leave and track your applications</p>
        </div>

        {/* Summary Cards */}
        <div className="grid gap-3 grid-cols-3">
          <Card>
            <CardContent className="pb-2.5 pt-2.5 text-center sm:pb-4 sm:pt-4">
              <div className="text-xl font-bold sm:text-2xl">{myRequests.length}</div>
              <div className="text-[10px] text-muted-foreground sm:text-xs">Total Requests</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pb-2.5 pt-2.5 text-center sm:pb-4 sm:pt-4">
              <div className="text-xl font-bold text-yellow-600 sm:text-2xl">{pendingCount}</div>
              <div className="text-[10px] text-muted-foreground sm:text-xs">Pending</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pb-2.5 pt-2.5 text-center sm:pb-4 sm:pt-4">
              <div className="text-xl font-bold text-green-600 sm:text-2xl">{approvedCount}</div>
              <div className="text-[10px] text-muted-foreground sm:text-xs">Approved</div>
            </CardContent>
          </Card>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger className="h-9 text-xs sm:h-10 sm:text-sm"><SelectValue placeholder="Select leave type" /></SelectTrigger>
                    <SelectContent>
                      {LEAVE_TYPES.map(t => (
                ...
                <div className="space-y-2">
                  <Label className="text-xs sm:text-sm">Total Days</Label>
                  <Input
              ...
              <Button type="submit" className="h-9 text-xs sm:h-10 sm:text-sm" disabled={createRequest.isPending || !formData.leave_type || !formData.start_date || !formData.end_date || hasBlockingConflict}>
              ...
              <table className="w-full text-xs sm:text-sm">
                <thead>
                  <tr className="border-b bg-muted/50">
                    <th className="px-2 py-2 text-left font-medium sm:px-4">Applied On</th>
                    <th className="px-2 py-2 text-left font-medium sm:px-4">Date Range</th>
                    <th className="px-2 py-2 text-left font-medium sm:px-4">Leave Type</th>
                    <th className="px-2 py-2 text-center font-medium sm:px-4">Days</th>
                    <th className="px-2 py-2 text-center font-medium sm:px-4">Status</th>
                    <th className="px-2 py-2 text-left font-medium sm:px-4">Remarks</th>
                    <th className="px-2 py-2 text-center font-medium sm:px-4">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {myRequests.map((req) => {
                    ...
                    return (
                      <tr key={req.id} className="border-b hover:bg-muted/30 transition-colors">
                        <td className="px-2 py-2 text-[10px] text-muted-foreground sm:px-4 sm:py-3 sm:text-xs">
                          {format(new Date(req.applied_at), 'dd MMM yyyy')}
                        </td>
                        <td className="px-2 py-2 sm:px-4 sm:py-3">
                          <div className="flex items-center gap-1 text-[11px] sm:gap-1.5 sm:text-sm">
                        ...
                        <td className="px-2 py-2 text-center text-[11px] sm:px-4 sm:py-3 sm:text-sm">{req.total_days}</td>
                        ...
                        <td className="max-w-[140px] truncate px-2 py-2 text-[10px] text-muted-foreground sm:max-w-[200px] sm:px-4 sm:py-3 sm:text-xs">
                        ...
                        <td className="px-2 py-2 text-center sm:px-4 sm:py-3">
                          {(req.status === 'Pending WSO' || req.status === 'Pending Supervisor') && (
                            <Button
                              size="sm"
                              variant="ghost"
                              className="h-6 text-[10px] text-destructive hover:text-destructive sm:h-7 sm:text-xs"
