...
import { Calendar, CalendarDays, FileText, Clock, Shield, Users, AlertTriangle, CheckCircle, XCircle, Award, Mail, Waves, Eye, Phone, MapPin, Hash, FileCheck, Globe, Star, ChevronLeft, ChevronRight, ArrowLeftRight, CalendarIcon, RefreshCw } from "lucide-react";
...
import { useDutyRoster, useRosterAssignments, useGridExtraDuties } from "@/hooks/useDutyGrid";
...
export default function EmployeeDashboard() {
  ...
  const [gridShift, setGridShift] = useState('Morning');
  const [gridTeam, setGridTeam] = useState('');
  const [gridPositionLabels, setGridPositionLabels] = useState<Record<string, string>>({});
  const gridIsNight = gridShift === 'Night';
  const gridDateStr = format(gridDate, 'yyyy-MM-dd');
  const { isLoading: gridEdgeLoading, refetch: refetchGrid } = useATCAssignments(gridDateStr, gridShift || undefined);
  const { data: gridRoster } = useDutyRoster(gridDate, gridShift, gridTeam);
  const { data: gridAssignments = [] } = useRosterAssignments(gridRoster?.id);
  const { data: gridExtraDuties = [] } = useGridExtraDuties(gridRoster?.id);
  ...
  const gridSections = useMemo(() => {
    const rows = gridIsNight ? POSITION_ROWS : POSITION_ROWS.filter((r) => !r.nightOnly);
  ...
  const getGridAssignment = (positionKey: string, department: string) =>
    gridAssignments.find((a) => a.position_name === positionKey && a.department === department);

  const gridActiveDepts = gridIsNight ? ALL_NIGHT_DEPARTMENTS : DEPARTMENTS;
  const gridActiveRows = gridIsNight ? POSITION_ROWS : POSITION_ROWS.filter((r) => !r.nightOnly);
  const gridMarkedCount = gridAssignments.filter((a) => a.employee_id).length;
  const gridTotalPositions = gridActiveRows.length * gridActiveDepts.length;
  const handleGridAssign = () => {};
  ...
  return (
    <DashboardLayout role="employee">
      <div className="space-y-4 md:space-y-6">
        ...
        <div className="space-y-4">
          <div>
            <h2 className="text-lg md:text-xl font-semibold text-gray-900 dark:text-gray-100">Shift Duty Grid</h2>
            <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">View position assignments for each shift</p>
          </div>
          <Card>
              ...
              <div className="flex flex-wrap items-center gap-4">
                <Popover>
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <CalendarPicker mode="single" selected={gridDate} onSelect={(d) => d && setGridDate(d)} className="p-3 pointer-events-auto" />
                  </PopoverContent>
                </Popover>
                <Select value={gridShift} onValueChange={setGridShift}>
                ...
                <Button variant="outline" size="sm" onClick={() => refetchGrid()} disabled={gridEdgeLoading}>
                  <RefreshCw className={`h-4 w-4 mr-1 ${gridEdgeLoading ? 'animate-spin' : ''}`} /> Sync
                </Button>
                <div className="ml-auto">
                  <Badge variant="secondary">Marked: {gridMarkedCount}/{gridTotalPositions}</Badge>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-0">
              {gridIsNight ? (
                <NightDutyGrid
                  ...
                  canEdit={false}
                  positionLabels={gridPositionLabels}
                  setPositionLabels={setGridPositionLabels}
                  getAssignment={getGridAssignment}
                  getAvailableEmployees={() => []}
                  handleAssign={handleGridAssign}
                />
              ) : (
                          ...
                          {section.rows.map((row, rowIndex) => (
                            ...
                            >
                              ...
                              {DEPARTMENTS.slice(0, row.deptCount || 3).map((dept) => {
                                const assignment = getGridAssignment(row.key, dept);
                                return (
                                  <React.Fragment key={dept}>
