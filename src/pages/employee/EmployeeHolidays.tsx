...
import { CalendarDays, LayoutList, ChevronDown, ChevronUp } from 'lucide-react';
...
function groupByMonth(list: any[]) {
    ...
    for (const h of list) {
        const k = h.holiday_date.substring(0, 7);
        if (!g[k]) g[k] = [];
        g[k].push(h);
    }
...
// ── Stable memoized Timeline Card outside component ──
            ...
            {isNext && (
                <div className="absolute -top-3 left-3 z-20">
                    <span className={`flex items-center gap-1.5 px-3 py-1 text-[9px] font-black uppercase tracking-wider rounded-full leading-none shadow-lg ${holiday.type === 'NH' ? 'bg-red-500 shadow-red-500/40' :
                        ...
                        Next Holiday in {daysUntil === 0 ? 'Today' : `${daysUntil} ${daysUntil === 1 ? 'Day' : 'Days'}`}
                    </span>
                </div>
            )}
...
export default function EmployeeHolidays() {
    ...
    // Single source of truth for holidays query
    ...
    const yearHolidays = useMemo(() =>
        holidays.filter((h: any) => {
            const y = new Date(h.holiday_date).getFullYear();
    ...
    const holidayDateSet = useMemo(() => {
        ...
        monthHolidays.forEach((h: any) => map.set(new Date(h.holiday_date).getDate(), h));
        return map;
    }, [monthHolidays]);
    ...
    const upcomingGroups = useMemo(() => groupByMonth(upcomingHolidays), [upcomingHolidays]);
    const pastGroups = useMemo(() => groupByMonth(pastHolidays), [pastHolidays]);
    ...
    return (
        <DashboardLayout role="employee">
            <div className="max-w-4xl mx-auto space-y-5">
                {/* ── Header ── */}
                <div>
                    <h1 className="text-2xl font-black tracking-tight">Holidays</h1>
                    {nextHoliday ? (
                        <p className="text-sm text-muted-foreground mt-0.5">
                            <span className="font-semibold text-foreground">{nextHoliday.name}</span>
                            {' • '}
                            <span>{format(new Date(nextHoliday.holiday_date), 'EEEE, d MMMM yyyy')}</span>
                            {' '}
                        ...
                        <p className="text-sm text-muted-foreground mt-0.5">Holiday calendar</p>
                    )}
                </div>

                {/* ── Unified Controls Bar: Timeline / Calendar + RH Toggle + Year ── */}
                <div className="flex flex-wrap items-center justify-between gap-2.5">
                    {/* View Switcher: Timeline | Calendar */}
                    <div className="inline-flex p-1 bg-slate-100 dark:bg-neutral-800/80 rounded-full border border-slate-200/80 dark:border-neutral-700/80">
                        <button
                            onClick={() => setTab('timeline')}
                            className={`flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-bold uppercase tracking-wider rounded-full transition-all ${tab === 'timeline'
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-bold uppercase tracking-wider rounded-full transition-all ${tab === 'calendar'
                                ? 'bg-white dark:bg-neutral-900 text-slate-900 dark:text-white shadow-sm'
                                : 'text-slate-500 hover:text-slate-900 dark:text-neutral-400 dark:hover:text-white'
                                }`}
                        >
                    ...
                    {/* Actions: RH Toggle & Year Selector */}
                    <div className="flex items-center gap-2">
                        {tab === 'timeline' && (
                            <button
                                onClick={() => setShowRH(!showRH)}
                                className={`flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-bold uppercase tracking-wider rounded-full border transition-all ${showRH
                            ...
                            >
                                <span className={`w-1.5 h-1.5 rounded-full ${showRH ? 'bg-white' : 'bg-amber-400'}`} />
                                RH {showRH ? 'ON' : 'OFF'}
                            </button>
                        )}
                        <Select value={String(selectedYear)} onValueChange={(v) => setSelectedYear(Number(v))}>
                            <SelectTrigger className="h-8 w-[92px] text-xs font-bold rounded-full bg-white dark:bg-neutral-800 border-slate-200 dark:border-neutral-700">
                                <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                        ...
                        {upcomingGroups.map(([monthKey, items], groupIdx) => {
                            ...
                            return (
                                <div key={monthKey} className="pb-8 relative pl-8">
                                    <div className={`sticky top-0 z-20 flex items-center py-2 relative mb-3 bg-background -mx-2 px-2 rounded-lg ${STICKY_HEADER_SHADOW}`}>
                                    ...
                                    <div className="space-y-3">
                                        {items.map((h: any, idx: number) => (
                                            <TimelineCard key={h.id} holiday={h} isNext={!showRH && groupIdx === 0 && idx === 0 && !!nextHoliday} daysUntil={nextHoliday?.daysUntil} />
                                        ))}
                                    </div>
                                </div>
                            );
