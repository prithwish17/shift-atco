...
import { LEAVE_TYPES, LEAVE_STATUS, getLeaveTypeLabel, getLeaveStatusInfo, YEAR_LOOKBACK } from '@/lib/leaveConstants';
...
export default function LeaveHistory() {
    const { user } = useAuth();
    const { data: requests = [], isLoading, isError: requestsError } = useMyLeaveRequests(user?.id);
    ...
    const { data: leaveRecords = [], isLoading: recordsLoading, isError: recordsError } = useLeaveRecords(empId, registerYear);
    ...
    const yearOptions = Array.from({ length: YEAR_LOOKBACK }, (_, i) => currentYear - i);

    if (requestsError || recordsError) {
        return (
                    ...
                    <Card className="border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950/30">
                        <CardContent className="flex items-center gap-3 py-4">
                            ...
                            <span className="text-sm text-red-700 dark:text-red-400">
                                Failed to load leave data. Please try refreshing the page.
                            </span>
                        </CardContent>
                    </Card>
                </div>
