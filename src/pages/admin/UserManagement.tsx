...
export default function UserManagement() {
  ...
  const { users, isLoading, error, approveUser, deleteUser, updateUserRole, toggleHideUser, isApproving, isDeleting, isUpdatingRole, isTogglingHide } = useUsers();
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const data = await invokeImportEmployees({ employees: parsedRecords });
          ...
          <CardContent>
            {error && (
              <div className="mb-4 rounded-md border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
                Unable to load users: {error instanceof Error ? error.message : "Unknown error"}
              </div>
            )}
            {isLoading ? (
