...
export default function SupervisorATCView() {
  ...
  const matchesSearch = (positionKey: string, department: string) => {
    ...
    const pos = POSITION_ROWS.find((r) => r.key === positionKey);
    if (pos?.label.toLowerCase().includes(q)) return true;
    const assignment = getAssignment(positionKey, department);
    if (assignment?.profiles?.full_name?.toLowerCase().includes(q)) return true;
    return false;
  };
  ...
  return (
    <DashboardLayout role="supervisor">
      <div className="space-y-4">
        ...
        {/* Extra Duties — Supervisor can manage */}
        <Card>
            ...
            <div className="divide-y">
              {extraDuties.map((duty) => (
                <div key={duty.id} className="flex items-center gap-2 py-2">
                  ...
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => deleteExtraDuty.mutate(duty.id)}>
                    <Trash2 className="h-3 w-3 text-destructive" />
                  </Button>
                </div>
              ))}
              {extraDuties.length === 0 && <div className="py-4 text-center text-muted-foreground text-sm">No extra duties</div>}
            </div>
          </CardContent>
        </Card>
