...
export default function RosterManagement() {
  ...
  const employees = useMemo<RosterEmployee[]>(() => {
    if (!users) return [];
    return users
      .filter((u) => !u.is_hidden && u.role !== "admin")
      .map((u) => ({
        id: u.id,
        empId: u.employee_id || "",
  ...
  const handleDutyChange = useCallback(async (empId: string, dutyDate: string, dutyCode: string) => {
    ...
    try {
      ...
      if (!emp) return;
      const previousDuty = scheduleMap.get(`${empId}::${dutyDate}`) || "";
      await updateScheduleMutation.mutateAsync({ employee: emp, dutyDate, dutyCode });
      logSupervisorEdit({
        action: "upsert",
        table: "employee_schedules",
        description: `Roster: changed duty ${previousDuty || "(none)"}→${dutyCode} for ${empId} on ${dutyDate}`,
        recordId: `${empId}|${dutyDate}`,
        before: { duty_code: previousDuty },
        after: { duty_code: dutyCode },
      });
    } finally {
              ...
              <div className="flex items-center gap-4 flex-wrap">
                <div className="w-full sm:flex-1 sm:min-w-[260px] sm:max-w-md relative">
                  ...
                  <Input
                    ...
                    className="pl-10 pr-8 bg-gray-50 border-gray-300 focus:border-gray-400"
                  />
                  {searchQuery && (
                    <Button
                      type="button"
