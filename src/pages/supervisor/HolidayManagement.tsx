  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingHoliday) {
        ...
        const result = await createHoliday.mutateAsync({ ...formData, type: formData.type as any, year, station: 'ALL', selectable: formData.type === 'RH', created_by: user.id });
        toast({ title: result.mode === "updated" ? "Existing holiday updated" : "Holiday created" });
      }
