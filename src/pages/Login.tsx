...
export default function Login() {
  ...
  const { signIn, user, userRole, loading, roleLoading, roleError } = useAuth();
  ...
  // True while the form submit is in flight OR sign-in succeeded but role is still resolving
  const isBusy = isLoading || (signInSucceeded && roleLoading);
  ...
  // Surface role errors (e.g. account not approved) after login attempt
  useEffect(() => {
    if (signInSucceeded && !roleLoading && roleError) {
      setAuthError(roleError);
      setSignInSucceeded(false);
    }
  }, [signInSucceeded, roleError, roleLoading]);
