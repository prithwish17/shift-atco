...
export default function BATestManagement() {
  ...
  // Duty codes that span morning + afternoon — show only in morning BA test list
  const MORNING_ONLY_COMBINED_CODES = new Set(["M+A", "A+M"]);
  ...
  const onDutyEmployees = useMemo(() => {
    ...
    return scheduleEntries
      .filter((entry) => {
        ...
        if (!matches.includes(teamDutyToday)) return false;

        // Employees with M+A / A+M appear only under morning, not afternoon
        const normalized = (entry.duty_code || "").toUpperCase().trim();
        if (MORNING_ONLY_COMBINED_CODES.has(normalized) && teamDutyToday === "A") {
          return false;
        }
