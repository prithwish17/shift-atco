...
export function normalizeCompOffEntries(items: unknown[]): CompOffHistoryEntry[] {
  ...
  // ── OPE per-month threshold: only the Nth duty onwards earns comp-off ──
  const OPE_SOURCES = new Set(["OPE", "OPE_DUTY"]);
  // Group OPE earned entries by duty month (only for months >= threshold)
  const opeByMonth = new Map<string, number[]>();
  entries.forEach((entry, idx) => {
    if (!OPE_SOURCES.has(entry.sourceType) || !entry.dutyDate) return;
    const month = entry.dutyDate.substring(0, 7); // "YYYY-MM"
    if (month < OPE_COMP_OFF_THRESHOLD_MONTH) return;
    if (!opeByMonth.has(month)) opeByMonth.set(month, []);
    opeByMonth.get(month)!.push(idx);
  });

  for (const [, indices] of opeByMonth) {
    // Sort by dutyDate so ordinal position is chronological
    indices.sort((a, b) => (entries[a].dutyDate || "").localeCompare(entries[b].dutyDate || ""));

    // First (OPE_COMP_OFF_MIN_DUTIES - 1) OPE duties in the month don't earn comp-off
    const ineligibleCount = OPE_COMP_OFF_MIN_DUTIES - 1;
    for (let i = 0; i < Math.min(ineligibleCount, indices.length); i++) {
      const entry = entries[indices[i]];
      if (entry.used) continue; // don't override already-used entries
      ...
      entry.remark = "Comp Off Not Available";
    }
  }
...
/**
 * Derives synthetic comp-off entries for CH (Closed Holiday) duties.
 * An employee earns a comp-off when they work an operational shift on a CH date.
 * A date is skipped if it is already covered by an earned, non-expired comp-off
 * from any other source.
 */
export function deriveCHDutyCompOffs(
  chHolidayDates: string[],
  rosterEntries: Array<{ isoDate: string; shift: string }>,
  existingEntries: CompOffHistoryEntry[],
): CompOffHistoryEntry[] {
  const today = startOfDay(new Date());

  // Dates already covered by an earned, non-expired comp-off — skip these
  const coveredDates = new Set(
    existingEntries
      .filter((e) => e.earned && e.status !== "expired")
      .map((e) => e.dutyDate)
      .filter((d): d is string => Boolean(d)),
  );

  // Index roster by ISO date for O(1) lookup
  const rosterByDate = new Map<string, string>();
  for (const entry of rosterEntries) {
    if (entry.isoDate) rosterByDate.set(entry.isoDate, entry.shift);
  }

  const results: CompOffHistoryEntry[] = [];

  for (const holidayDate of chHolidayDates) {
    if (coveredDates.has(holidayDate)) continue;

    const shift = rosterByDate.get(holidayDate);
    if (!shift) continue;

    const normalizedShift = shift.replace(/\s+/g, "").toUpperCase();
    if (!COMP_OFF_ELIGIBLE_DUTY.has(normalizedShift)) continue;
    ...
    const expired = expiryDate != null && (daysRemaining ?? 0) < 0;
    const status: CompOffHistoryEntry["status"] = expired ? "expired" : "available";
    const remark = expired
      ? `Expired on ${expiryDate}`
      : expiryDate
        ? `${daysRemaining ?? 0} day${daysRemaining === 1 ? "" : "s"} remaining`
        : "Available";
    ...
    results.push({
      dutyDate: holidayDate,
      leaveApplied: null,
      dutyPerformed: normalizedShift,
      sourceType: "CH_DUTY",
      sourceLabel: "Closed Holiday Duty",
      expiryDate,
      daysRemaining,
      hideDates: false,
      eligible: true,
