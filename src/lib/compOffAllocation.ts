...
import { OPE_COMP_OFF_MIN_DUTIES, OPE_COMP_OFF_THRESHOLD_MONTH } from "@/lib/leaveConstants";
...
export function buildCompOffAllocationCandidates(rows: CompOffLedgerRow[]): CompOffAllocationCandidate[] {
  ...
  // ── OPE per-month threshold (mirrors normalizeCompOffEntries logic) ──
  ...
  candidates.forEach((entry, idx) => {
    ...
    const month = entry.dutyDate.substring(0, 7);
  ...
  for (const [, indices] of opeByMonth) {
    indices.sort((a, b) => (candidates[a].dutyDate || "").localeCompare(candidates[b].dutyDate || ""));
    const ineligibleCount = OPE_COMP_OFF_MIN_DUTIES - 1;
    for (let i = 0; i < Math.min(ineligibleCount, indices.length); i++) {
      const entry = candidates[indices[i]];
  ...
  return candidates.sort(compareCompOffCandidates);
}
