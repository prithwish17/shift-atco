...
export function useCreateDutyExchange() {
  ...
  return useMutation({
    ...
    onError: (error: Error) => {
      logCriticalEvent('duty_exchange_conflict', {
        error: error.message,
      });
    },
    onSuccess: () => {
...
export function useProcessExchangeApproval() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      logSupervisorEdit({
        action: "update",
        table: "duty_exchange_approvals",
        description: `${variables.action === "approve" ? "Approved" : "Rejected"} duty exchange ${variables.request_id}`,
        recordId: variables.request_id,
        after: { action: variables.action, remarks: variables.remarks || null },
      });
      queryClient.invalidateQueries({ queryKey: ["duty_exchanges"] });
      queryClient.invalidateQueries({ queryKey: ["exchange_approvals"] });
...
export function useUpdateDutyExchange() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      logSupervisorEdit({
        action: "update",
        table: "duty_exchanges",
        description: `Updated duty exchange ${(data as any)?.id || "unknown"} status to ${(data as any)?.status || "unknown"}`,
