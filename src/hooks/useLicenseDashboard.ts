...
function isActiveRatingEntry(entry?: RatingDataEntry | null) {
...
function hasValidAccSRating(ratingData: Record<string, RatingDataEntry>) {
    ...
    const validityWindow = getRatingValidityWindow(latestProficiency.date, accsEntry.endorsement_date || null);
    return Boolean(validityWindow?.validUpto && getHealthFromDate(validityWindow.validUpto).status !== 'expired');
}

function getRatingValidityWindowForEntry(
    ratingKey: string,
    ...
    const validityWindow = getRatingValidityWindow(latestProficiency.date, entry.endorsement_date || null);
    if (!validityWindow) return null;
...
export function buildEmployeeLicenseHealth(profile: any, licensesInput: LicenseWithExtras[] = []): EmployeeLicenseHealth {
    ...
    const ratings = Object.entries(ratingData)
        .map(([ratingKey, rawEntry]) => {
            ...
            const latestProficiency = getLatestProficiencyFromHistory(entry);
            const isActive = isActiveRatingEntry(entry);
            const validityWindow = isActive
                ? getRatingValidityWindowForEntry(ratingKey, entry, ratingData)
                : null;
            const ratingHealth = validityWindow?.exemptByAccS
                ? { status: 'valid' as HealthStatus, daysUntil: null }
                : validityWindow?.validUpto
            ...
            return {
                ...
                subtitle: isActive
                    ? validityWindow?.exemptByAccS
                        ? 'PLR validity covered by an active ACC(S) rating'
                        : validityWindow?.validUpto
    ...
    const reviewCandidates = [...activeRatingsWatchlist, ...licensesList]
        .filter((item) => item.daysUntil !== null);
    const upcomingReviewCandidates = reviewCandidates
        .filter((item) => (item.daysUntil ?? Number.NEGATIVE_INFINITY) >= 0);
    ...
    const latestExpiry = [...(upcomingReviewCandidates.length > 0 ? upcomingReviewCandidates : reviewCandidates)]
