...
export function expiryAlertEmail(data: {
  ...
  const body = `
    <p>Hi ${escapeHtml(data.employeeName)},</p>
    <p>Your <strong>${escapeHtml(data.itemType)}</strong> expires on
       <strong>${escapeHtml(data.expiryDate)}</strong>
       (<span style="${urgency}">${data.daysUntil} day${data.daysUntil !== 1 ? 's' : ''} remaining</span>).</p>
    <p>Please take the necessary action to renew it before expiry.</p>`
...
export function compOffExpiryEmail(data: {
  ...
  const body = `
    <p>Hi ${escapeHtml(data.employeeName)},</p>
    <p>Your comp-off${data.dutyDate ? ` earned on <strong>${escapeHtml(data.dutyDate)}</strong>` : ''}
       expires in <strong>${data.daysAhead} days</strong> (${escapeHtml(data.expiryDate)}).</p>
    <p>Apply for it before it expires.</p>`

  return {
    subject: `Comp-Off Expiry: ${data.daysAhead} days left`,
    html: baseLayout('Comp-Off Expiry Alert', body, `${data.appUrl}/employee/comp-off`, 'Apply Comp-Off'),
  }
}
  ...
  const body = `
    ...
    <table style="width:100%;border-collapse:collapse;margin:16px 0">
      <tr><td style="padding:8px 12px;background:#fef2f2;border:1px solid #e5e7eb;font-weight:600;width:120px">Previous</td>
          <td style="padding:8px 12px;border:1px solid #e5e7eb;text-decoration:line-through;color:#9ca3af">${escapeHtml(data.oldDutyCode)}</td></tr>
      <tr><td style="padding:8px 12px;background:#f0fdf4;border:1px solid #e5e7eb;font-weight:600">New</td>
          <td style="padding:8px 12px;border:1px solid #e5e7eb;font-weight:600;color:#16a34a">${escapeHtml(data.newDutyCode)}</td></tr>
    </table>`

  return {
    subject: `Duty Changed: ${data.dutyDate} — ${data.oldDutyCode} → ${data.newDutyCode}`,
