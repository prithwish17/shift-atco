...
const ENDPOINT = '/functions/v1/process-notification-queue'
const BATCH_SIZE = 50
  ...
  switch (p.template || job.event_type) {
    ...
    case 'license_expiry':
      return expiryAlertEmail({
        employeeName: userName,
        itemType: templateData.item_type || p.metadata?.type || 'License',
    ...
    case 'duty_change':
      return dutyChangeEmail({
        employeeName: userName,
        dutyDate: templateData.duty_date || '',
        oldDutyCode: templateData.old_duty_code || '',
        newDutyCode: templateData.new_duty_code || '',
        appUrl,
      })
...
Deno.serve(async (_req: Request) => {
  ...
  try {
    ...
    console.error('[process-notification-queue]', err)
  }
