...
Deno.serve(async (req) => {
  try {
    const { user_ids, title, body, url, category, metadata, emailTemplate, emailData, sendEmail, emailPriority } = await req.json()
    ...
    const result = await notifyUsers({
      ...
      metadata,
      emailTemplate: emailTemplate || category || 'general',
      emailData: emailData || {},
      sendEmail: sendEmail !== undefined ? sendEmail : true,
      emailPriority: emailPriority || 5,
    })
