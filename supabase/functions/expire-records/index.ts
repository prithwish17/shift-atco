  ...
  try {
    ...
    for (const co of (expiringCompOffs || []) as any[]) {
      ...
      await notifyUsers({
        ...
        body: `Your comp-off${co.duty_date ? ' earned on ' + formatDate(co.duty_date) : ''} has expired (expired on ${co.expiry_date ? formatDate(co.expiry_date) : 'today'}). Please note it can no longer be availed.`,
        url: '/employee/comp-off',
        category: 'compoff_expired',
        metadata: { duty_date: co.duty_date, expiry_date: co.expiry_date },
        emailTemplate: 'compoff_expired',
        emailData: {
          duty_date: co.duty_date ? formatDate(co.duty_date) : undefined,
          expiry_date: co.expiry_date ? formatDate(co.expiry_date) : 'today',
        },
      })
    }
    ...
    for (const lic of (expiringLicenses || []) as any[]) {
      ...
      await notifyUsers({
        ...
        body: `Your ${(lic.license_type || 'license').toUpperCase()} license has expired (expired on ${lic.expiry_date ? formatDate(lic.expiry_date) : 'today'}). Please renew it at the earliest.`,
        url: '/employee/licenses',
        category: 'license_expired',
        metadata: { license_type: lic.license_type, expiry_date: lic.expiry_date, item_type: (lic.license_type || 'License').toUpperCase() + ' License' },
        emailTemplate: 'license_expired',
        emailData: {
          item_type: (lic.license_type || 'License').toUpperCase() + ' License',
          expiry_date: lic.expiry_date ? formatDate(lic.expiry_date) : 'today',
        },
      })
    }
    ...
    for (const ue of (expiringEndorsements || []) as any[]) {
      ...
      await notifyUsers({
        ...
        body: `Your ${ue.position || ''} endorsement${ue.airport ? ' at ' + ue.airport : ''} has expired (expired on ${ue.expiry_date ? formatDate(ue.expiry_date) : 'today'}). Please renew it at the earliest.`,
        url: '/employee/licenses',
        category: 'license_expired',
        metadata: { position: ue.position, airport: ue.airport, expiry_date: ue.expiry_date, item_type: `${ue.position || 'Unit'} Endorsement${ue.airport ? ' (' + ue.airport + ')' : ''}` },
        emailTemplate: 'license_expired',
        emailData: {
          item_type: `${ue.position || 'Unit'} Endorsement${ue.airport ? ' (' + ue.airport + ')' : ''}`,
          expiry_date: ue.expiry_date ? formatDate(ue.expiry_date) : 'today',
        },
      })
    }
    ...
    if (expiringMedCerts?.length) {
      ...
      for (const mc of expiringMedCerts as any[]) {
        ...
        await notifyUsers({
          ...
          body: `Your ${mc.medical_class || ''} medical certificate has expired (expired on ${mc.expiry_date ? formatDate(mc.expiry_date) : 'today'}). Please renew it at the earliest.`,
          url: '/employee/licenses',
          category: 'license_expired',
          metadata: { medical_class: mc.medical_class, expiry_date: mc.expiry_date, item_type: `${mc.medical_class || ''} Medical Certificate` },
          emailTemplate: 'license_expired',
          emailData: {
            item_type: `${mc.medical_class || ''} Medical Certificate`,
            expiry_date: mc.expiry_date ? formatDate(mc.expiry_date) : 'today',
          },
        })
      }
    }
