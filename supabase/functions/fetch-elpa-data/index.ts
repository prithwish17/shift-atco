...
function parseJsDate(raw: string | null | undefined): string | null {
...
Deno.serve(async (req) => {
    ...
    const jobName =
        ...
        (typeof requestBody?.__cron_job_name === "string" ? requestBody.__cron_job_name : "fetch-elpa-data");
    ...
    try {
        ...
        try {
            const { data: setting } = await adminClient
                .from("app_settings")
                .select("value")
                .eq("key", "elpa_data_webapp_url")
                .maybeSingle();
            if (setting?.value) {
                elpaUrl = setting.value;
            }
        } catch {
            // no fallback
        }
        ...
        if (!elpaUrl) {
            const errMsg = "ELPA webapp URL not configured in System Settings";
        ...
        const response = await fetch(elpaUrl, {
            method: "GET",
            redirect: "follow",
            headers: {
        ...
        if (!response.ok) {
            const errMsg = `ELPA webapp returned ${response.status}`;
            await logApiCall("error", errMsg, Date.now() - startTime, triggeredBy);
            throw new Error(errMsg);
        }

        const json = await response.json();
        const rawRecords: ElpaRecord[] = Array.isArray(json)
            ? json
        ...
        if (!Array.isArray(rawRecords)) {
            const errMsg = "Unexpected response format from ELPA webapp";
            await logApiCall("error", errMsg, Date.now() - startTime, triggeredBy);
            throw new Error(errMsg);
        }

        // --- Transform & upsert ---
        const batchId = `elpa-sync-${Date.now()}`;
        ...
        const records = rawRecords
            ...
            .map((r) => ({
                ...
                elpa_level: r.level ? String(r.level).trim() : null,
                elpa_valid_upto: parseJsDate(r.valid_upto),
                elpa_endorsed_upto: parseJsDate(r.endorsed_upto),
                elpa_synced_at: now,
        ...
        if (records.length > 0) {
            ...
            for (let i = 0; i < records.length; i += BATCH_SIZE) {
                ...
                if (upsertError) {
                    console.error("ELPA data upsert error:", upsertError);
                    throw upsertError;
                }
        ...
        const successMsg = `Fetched ${rawRecords.length} ELPA records, upserted ${upserted}`;
