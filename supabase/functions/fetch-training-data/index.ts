...
Deno.serve(async (req) => {
    ...
    const jobName =
        ...
        (typeof requestBody?.__cron_job_name === "string" ? requestBody.__cron_job_name : "fetch-training-data");
    ...
    try {
        ...
        if (records.length > 0) {
            ...
            // Merge incoming data with existing so the latest sheet values win,
            // while still retaining any extra DB-only keys that are missing from the feed.
            const mergedRecords = records.map((record) => {
                ...
                return {
                    ...record,
                    ojti: { ...(existing.ojti || {}), ...(record.ojti as Record<string, any>) },
                    examiner: { ...(existing.examiner || {}), ...(record.examiner as Record<string, any>) },
                    completion_dates: { ...(existing.completion_dates || {}), ...(record.completion_dates as Record<string, any>) },
                    instructor_validity: { ...(existing.instructor_validity || {}), ...(record.instructor_validity as Record<string, any>) },
                    examiner_validity: { ...(existing.examiner_validity || {}), ...(record.examiner_validity as Record<string, any>) },
                };
            });
